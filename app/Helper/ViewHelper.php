...
function get_new_user()
{
	...
	return User::where('is_read',0)->orderBy('id', 'desc')->limit(3)->get();
}
...
function get_new_provider()
{
	...
	return Provider::where('is_read',0)->orderBy('id', 'desc')->limit(3)->get();
}

function get_complaint(){
    return Complaint::orderBy('id', 'desc')->limit(3)->get();
}
function get_new_open_ticket(){

	$mytime = Carbon::now();
	
    return Complaint::whereDate('created_at',$mytime->toDateString())->where('transfer',2)->where('status',1)->get();
}
...
function registrationMail($type,$email,$name,$subject){  
...
function InvoiceMail($type,$email,$name,$subject){  
    $temp_data = EmailTemplate::where('type',$type)->first();
    $template =  str_ireplace('{{USER_NAME}}',$name,$temp_data->template);
    $template =  str_ireplace('{{DATE}}',date('d-m-Y'),$template);  
}
function supportMail($type,$email,$name,$subject){  
    $temp_data = EmailTemplate::where('type',$type)->first();
    $template =  str_ireplace('{{USER_NAME}}',$name,$temp_data->template);
    $template =  str_ireplace('{{DATE}}',date('d-m-Y'),$template);  
}
function ticketMail($type,$email,$name,$subject){  
    $temp_data = EmailTemplate::where('type',$type)->first();
    $template =  str_ireplace('{{USER_NAME}}',$name,$temp_data->template);
    $template =  str_ireplace('{{DATE}}',date('d-m-Y'),$template);  
}
function allMail($type,$email,$name,$subject){  
    $temp_data = EmailTemplate::where('type',$type)->first();
    $template =  str_ireplace('{{USER_NAME}}',$name,$temp_data['template']);    
    //$template =  str_ireplace('{{USER_NAME}}',$name,$temp_data->template); -  0430 - jlock
    $template =  str_ireplace('{{DATE}}',date('d-m-Y'),$template); 
    return $template; 
}

function sendMail($type,$email,$name,$subject,$data=null){ 
    if($type=='Invoice'){
           $template =  InvoiceMail($type,$email,$name,$subject,$data);
    }else{
        $template =  allMail($type,$email,$name,$subject);
    }   
