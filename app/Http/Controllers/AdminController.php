...
public function allocation_list()
    ...
    public function settings_store(Request $request)
    {
        ...
        Setting::set('provider_max_money', $request->provider_max_money);
    ...
    public function payment($type)
    {
        try {
            ...
            return view('admin.payment.payment-history', compact('payments','title'));
        } catch (Exception $e) {
             return back()->with('flash_error','Something Went Wrong!');
        }
    }
    ...
    public function taxCompany(Request $request){
        $commission_percentage = Setting::get('commission_percentage');
        $tax_percentage = Setting::get('tax_percentage');

        $requests = UserRequests::where('status','COMPLETED')->with('user','provider','payment')->get();
        //$total_income = UserRequestPayment::withsum('total');
        //$total_commission = ($total_income * $commission_percentage)/100;
        $company_data = DB::select(DB::raw("SELECT SUM(ROUND(user_request_payments.total)) AS 'total',SUM(ROUND(user_request_payments.commision)) as 'commission',SUM(ROUND(user_request_payments.fixed) + ROUND(user_request_payments.distance)) as overall  FROM `user_requests` join user_request_payments where user_request_payments.request_id = user_requests.id and status='COMPLETED'"))[0];
        $total_tax = ($company_data->commission * $tax_percentage)/100;
        return view('admin.tax.tax',compact('requests','commission_percentage','total_income','total_commission','company_data','total_tax','tax_percentage'));
    }
    public function taxDriver(Request $request){
        $commission_percentage = Setting::get('commission_percentage');
        $tax_percentage = Setting::get('tax_percentage');
        $requests = UserRequests::where('status','COMPLETED')->with('user','provider','payment')->get();
        //$total_income = UserRequestPayment::withsum('total');
        //$total_commission = ($total_income * $commission_percentage)/100;
        /*$company_data = DB::table('user_requests')
                     ->select(DB::raw('count(*) as user_requests'))
        ...
        dd($company_data);*/
           $company_data = DB::select(DB::raw("SELECT SUM(ROUND(user_request_payments.total)) AS 'total',SUM(ROUND(user_request_payments.commision)) as 'commission',SUM(ROUND(user_request_payments.fixed) + ROUND(user_request_payments.distance)) as overall  FROM `user_requests` join user_request_payments where user_request_payments.request_id = user_requests.id and status='COMPLETED'"))[0];
        $total_tax = ($company_data->commission * $tax_percentage)/100;
        return view('admin.tax.drivertax',compact('requests','commission_percentage','total_income','total_commission','company_data','total_tax','tax_percentage'));
    }
    public function taxUser(Request $request){
          $commission_percentage = Setting::get('commission_percentage');
        $tax_percentage = Setting::get('tax_percentage');
        ...
        $company_data = DB::select(DB::raw("SELECT SUM(ROUND(user_request_payments.total)) AS 'total',SUM(ROUND(user_request_payments.commision)) as 'commission',SUM(ROUND(user_request_payments.fixed) + ROUND(user_request_payments.distance)) as overall  FROM `user_requests` join user_request_payments where user_request_payments.request_id = user_requests.id and status='COMPLETED'"))[0];
        $total_tax = ($company_data->commission * $tax_percentage)/100;
        return view('admin.tax.usertax',compact('requests','commission_percentage','total_income','total_commission','company_data','total_tax','tax_percentage'));
    }
    ...
    public function changevendorpassword(Request $request)
    {
        if(Setting::get('demo_mode', 0) == 1) {
            return back()->with('flash_error','Disabled for demo purposes! Please contact us at info@appoets.com');
        }
        ...
        try {
            ...
            $provider = Fleet::findOrFail($request->id);
            if( !$provider  ) {
                throw new Exception('Provider Not Found');
            }
    ...
    public function fare_settings_create()
    {
        ...
        return view('admin.fareSetting.create',compact('data','countries','zones'));
    }
    ...
    public function editFare($id){
        ...
        return view('admin.settings.fare_edit',compact('data','countries','zones'));
    }
    ...
    public function editFareAction(Request $request){
        ...
        if($faresetting = FareSetting::find($request->id)){
            ...
            $faresetting->is_rental = $request->is_rental;
            $faresetting->minutes = $request->is_rental==1?$request->minutes:0;
    ...
    public function cabAllocation_edit($id){
       ...
       // }
        return view('admin.service.allocation_edit',compact('package','services','zones','fare','ridetypes','countries'));
    }
     ...
     public function cabAllocation_update(Request $request){
       ...
       $package->ride_type_id =$request->ride_type_id; 
       $package->save();
    ...
    public function lost_management(){
        $data = LostItem::get();
        return view('admin.lost_management', compact('data'));
    }
    
    public function lost_edit($id){
       try {
           $data =  LostItem::find($id)->first();
            return view('admin.lost_edit', compact('data'));
        } 
        catch (Exception $e) {
            return back()->with('flash_error', 'User Not Found');
        }
    ...
    public function lost_update(Request $request,$id){
       try {
            $data = LostItem::where('id',$id)->first();
            $data->status = $request->status;
            $data->save();
            return back()->with('flash_success', 'Lost item updated successfully');
        } 
        catch (Exception $e) {
            return back()->with('flash_error', 'User Not Found');
        }
