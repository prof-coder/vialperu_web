...
class SocialLoginController extends Controller
{
    ...
    public function facebookViaAPI(Request $request) { 
        ...
        try{
            ...
            if($AuthUser){
                ...
                if($AuthUser->referral_code==null){
                    $AuthUser->referral_code    =   $this->referralCodeGenerator($AuthUser->first_name);
                }
                $AuthUser->save();  
				...
				$AuthUser->login_by			=	"facebook";
                $AuthUser->referral_code    =   $this->referralCodeGenerator($first_name);
    ...
    public function referralCodeGenerator($name){
        $name = trim($name);
        $res = strtoupper(substr($name,0,4));
                ...
                if($guard == 'provider') {
                    ...
                    if($google->id){
                        ...
                        if($AuthUser){ 
                            ...
                            $new->login_by="google";
                            $new->referral_code    =   $this->referralCodeGenerator($name[0]);
