...
class LiveTrip extends Controller
{
    ...
    public function driverDetails($id){
        ...
        $details = "https://maps.googleapis.com/maps/api/geocode/json?latlng={$provider->latitude},{$provider->longitude}&key=".env('GOOGLE_MAP_KEY');
            ...
            $provider_img = $provider->avatar!=''?'http://13.59.90.76/storage/app/public/'.$provider->avatar:'http://quickrideja.com/storage/app/public/provider/profile/user.png';
        return   '<div id="siteNotice">'.
            '</div>'.
            '<div id="bodyContent" style="width:250px;">'.
            '<div style="width:100%" class="row">'.
                '<div style="width:100px%;float:left;padding-left:20px;">'.
                    '<img src="'.$provider_img.'" style="width:70px;height:70px;border-radius:50%;">'.
                '</div>'.
                '<div style="position: relative; top: 16px; text-align:center;">'.
    ...
    public function ongoingDetail($id){
        ...
        //  ->where('request_filters.request_id', $rq->id)->first();
         $provider = Provider::rightJoin('user_requests', 'user_requests.provider_id','=','providers.id')
         ->where('user_requests.id', $rq->id)->first();
             $provider_img = $provider->avatar!=''?'http://13.59.90.76/storage/app/public/'.$provider->avatar:'http://quickrideja.com/storage/app/public/provider/profile/user.png';
        return   '<div id="siteNotice">'.
            '</div>'.
            '<div id="bodyContent" style="width:250px;">'.
    ...
    public function completDetails($id){
        ...
        $provider = Provider::rightJoin('user_requests', 'user_requests.provider_id','=','providers.id')
         ...
         return   '<div id="siteNotice">'.
            ...
            '<div style="width:100%" class="row">'.
                '<div style="width:100px%;float:left;padding-left:20px;">'.
                    '<img src="'.($user->picture!=''?$user->picture:'http://quickrideja.com/storage/app/public/provider/profile/user.png').'" style="width:70px;height:70px;border-radius:50%;">'.
                '</div>'.
                '<div style="position: relative; top: 16px; text-align:center;">'.
                    "<h5>".($user->first_name ?? 'Passanger')."</h5>".
                    "<p>Passanger</p>".
    ...
    public function getDetailsD($type,$id){
        ...
        public function ongoingDetailD($id){
            $rq =  UserRequests::find($id);
            $user = User::find($rq->user_id);
            $provider = Provider::rightJoin('user_requests', 'user_requests.provider_id','=','providers.id')->where('user_requests.id', $rq->id)->first();
             $provider_img = $provider->avatar!=''?'http://13.59.90.76/storage/app/public/'.$provider->avatar:'http://quickrideja.com/storage/app/public/provider/profile/user.png';
            return   '<div id="siteNotice">'.
                '</div>'.
                '<div id="bodyContent" style="width:250px;">'.
