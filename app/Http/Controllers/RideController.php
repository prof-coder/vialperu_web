...
class RideController extends Controller
{
    ...
    public function create_ride(Request $request)
    {
        ...
        }   elseif(isset($data->getData()->message)){
            
            $trips = $this->UserAPI->upcoming_trips();
            return view('user.ride.upcoming',compact('trips'));
            //return redirect()->back()->with('message', $data->getData()->message ? $data->getData()->message:$data->getData()->error);
            
        }   elseif(isset($data->getData()->error)){
    ...
    /**public function create_ride(Request $request)
    {
        ...
        if(isset($data->getData()->flash_error)){
            return redirect()->back()->with('message', $data->getData()->flash_error ? $data->getData()->flash_error:$data->getData()->error);
        }
