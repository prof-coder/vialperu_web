           ...
           <!--  <a href="{{ route('admin.provider.index') }}" class="btn btn-default pull-right"><i class="fa fa-angle-left"></i> Back</a> -->
            ...
            <form class="form-horizontal" action="{{route('admin.provider.update', $provider->id )}}" method="POST" enctype="multipart/form-data" role="form">
                ...
                <div class="form-group row">
                    <label for="first_name" class="col-xs-2 col-form-label">Select Vendor</label>
                    <div class="col-xs-10">
                        <select class="form-control" name="fleet" id="fleet">
                            <option value=" ">-- Select Vendor --</option>
                            @foreach($fleets as $index =>$fl)                    
                                <option <?php if($provider->fleet==$fl->id){ echo "Selected";}; ?> value="{{$fl->id}}">{{$fl->name}}</option>
                            }
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="form-group row">
                    <label for="money" class="col-xs-2 col-form-label">Advance Money</label>
                    <div class="col-xs-10">
                        <input class="form-control" type="text" value="{{ $provider->money }}" name="money" required id="money" placeholder="money">
                    </div>
                </div>                
                <div class="form-group row">
