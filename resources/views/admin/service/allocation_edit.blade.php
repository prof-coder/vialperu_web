        ...
        <div class="box box-block bg-white">
            ...
            <form class="form-horizontal" action="{{route('admin.cabAllocation_update')}}" method="GET" enctype="multipart/form-data" role="form">
                ...
                <div class="form-group row">
                    <label for="provider_name" class="col-xs-12 col-form-label">Ride Type</label>
                    <div class="col-xs-10">
                        <select class="form-control" type="text" name="ride_type_id" id="ride_type_id" required>
                            <option value="">------Select-------</option>
                            @foreach($ridetypes as $k=>$ridetype)
                            <option value="{{$ridetype->id}}" {{$ridetype->id==$package->ride_type_id ? 'selected' : ''}}>{{$ridetype->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="form-group row">
                    <label for="provider_name" class="col-xs-12 col-form-label">Country List</label>
                    <div class="col-xs-10">
                        <select class="form-control" type="text" name="country_id" id="serviceType" required>
                            <option value="">------Select-------</option>
                            @foreach($countries as $k=>$country)
                            <option value="{{$country->id}}" <?php if(@$package->country_id==$country->id){echo "Selected";}?>>{{$country->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                 <div class="form-group row">
                    ...
                    <div class="col-xs-10">
                        <select class="form-control" type="text" name="cab_id" id="serviceType" required>
                            <option value="">------Select-------</option>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-xs-10">
                        <select class="form-control" type="text" name="fare_setting_id" id="serviceType" required>
                            <option value="">------Select-------</option>
                            @foreach($fare as $k=>$v)
